 ...
 *      change a shared colour value.
 *
 * APIs used:
 *   ExtensionBuilder  — @tridion-sites/extensions
 *     The fluent builder passed to the `initialize` lifecycle hook.  It exposes
 *     the `contentEditor` namespace which contains methods for registering
 *     Content Editor extension points.
 *
...
export const registerCustomEditorWithField = (builder: ExtensionBuilder) => {
    ...
    // state back up the tree without prop-drilling.
    builder.contentEditor.addItemEditor(() => {
        return {
            ...
            useContentEditorView: () => {
                return {
                    isAvailable: true,
                    // Falls back to the SDK default priority when not configured.
    ...
    // EditorContext to push a colour value up to the EditorContextProvider.
